...
# 쓰레드 클래스        
class EachId(threading.Thread):
    thread_connectkey = ""
    thread_secretkey = ""

    def __init__(self, connectkey, secretkey):
        self.thread_connectkey = connectkey
        self.thread_secretkey = secretkey
    ...
    def run(self):
        try:
            ...
            api = XCoinAPI(self.thread_connectkey, self.thread_secretkey)
            volume = 0
            while True:
                if volume > goal:
                    break
...
def start(currency, goal, qty):
    for connectkey, secretkey in list:
        each_thread = EachId(connectkey, secretkey)
        each_thread.start()
